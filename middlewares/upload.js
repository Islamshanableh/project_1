...
exports.uploadCatlog = multer({ storage, limits: 10 }).single('catalog');
...
const storage_ = multer.diskStorage({
  destination(_req, _file, cb) {
    ...
    const fileExits = existsSync(
      resolve(process.cwd(), `./tmp/my-uploads/${_file.originalname}.xlsx`),
    );
...
exports.upload = multer({ storage: storage_ }).any();
